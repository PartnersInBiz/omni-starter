...
const { page, compAdd } = useBuilders();
...
const submit_form = (data: any) => {
  console.log("subit front", data);
  showEdit.value = false;
  compAdd(edit.value);
};

const updated_form = (data: any) => {
  console.log("updatessss", data);
...
<template>
  <li class="mb-1 min-h-[2rem] items-center">
    ...
    >
      <div v-for="(comp, index) in subsidebarclean" :key="index">
        <div class="flex items-center justify-between text-muted-400">
          ...
          <div class="space-x-2">
            <Icon
              :name="compShows.includes(comp.name) ? 'mdi:eye-off' : 'mdi:eye'"
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <template
                    v-for="(slot2, index) in slot.slots"
                    :key="index"
